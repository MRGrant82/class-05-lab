...
nd the third and fourth elements are strings that EXACTLY follow this example and use the values that were input into the function:
...
function sumAndMultiply(a, b, c) { 
  ...
  let sumVar = sum(a, b, c);
  let productVar = product(a, b, c);
  
  let message1 = `${a} and ${b} and ${c} sum to ${sumVar}.`;
  let message2 = `The product of ${a} and ${b} and ${c} is ${productVar}.`;
  
  return [sumVar, productVar, message1, message2];
}
...
that you've already created. You're going to have to be resourceful to figure out how to do this. However, you may continue to use 
...
function sumArray(sumArr) {
  ...
  for (let i = 0; i < sumArr.length; i++) {
    // This iterates through each element of the array above and will call my sum function to add them together
    total = sum(total, sumArr[i]);
  }
    // We still use the `` and use join which concatenates the array into a string
  let message = `${sumArr.join(',')} was passed in as an array of numbers, and ${total} is their sum.`;
  // Why is the console printing twice?
  console.log(message);
...
// Write your code here
// Multiply function - I was confused and learned that you don't need to multiply them all here, we multiply later
function multiply(a, b) {
  return a * b;
}
...
function multiplyArray(arr) {
  ...
  console.log([result, `The numbers ${arr.join(',')} have a product of ${result}.`]);
  return [result, `The numbers ${arr.join(',')} have a product of ${result}.`];
}
